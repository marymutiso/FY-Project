...
$sql1 = "SELECT refundID, clientID, clientFname, clientSname, jobID, refundAmount, clientPhone, refundStatus FROM tblrefunds ORDER BY refundID ASC"; // using mysqli_query instead
				...
				<div class="col-md-2">
					<div class="dropdown create">
					  ...
					  <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
					    <li><a href="#">Add Refund record</a></li>
...
<!--<a href="add.html">Add New Data</a><br/><br/>-->
	...
	<table width='100%' style="border: ">
		<caption><legend style="text-align: center; position: relative;">Pending Clients Refunds</legend></caption>
	<tr bgcolor='#CCCCCC'>
		<th>Refund ID</td>
		<th>Client's ID</td>
		<th colspan="2">clients's name</td>
		<th>Order Number</td>
		<th>Amount payable</td>
		<th>Client's phone no.</td>
		<th>Cefund Status</td>
	...
	while($res = mysqli_fetch_array($result)) { 		
		echo "<tr>";
		echo "<td>".$res['refundID']."</td>";
		echo "<td>".$res['clientID']."</td>";
		echo "<td>".$res['clientFname']."</td>";
		echo "<td>".$res['clientSname']."</td>";
		echo "<td>".$res['jobID']."</td>";
		echo "<td>".$res['refundAmount']."</td>";
		echo "<td>".$res['clientPhone']."</td>";
		echo "<td>".$res['refundStatus']."</td>";					
		echo "<td width='30'><a href=\"editrefunds.php?id=$res[refundID]\">Edit</a> | <a href=\"deleterefunds.php?id=$res[refundID]\" onClick=\"return confirm('Are you sure you want to cancel the refund?')\">Delete</a></td>";
		echo "</tr>";		
	}/*else{
		 printf("Error: %s\n", mysqli_error($db));
	}*/
